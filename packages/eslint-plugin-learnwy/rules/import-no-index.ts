...
const importNoIndex: TSESLint.RuleModule<"outImportNoIndex" | "innerImportIndex" | "indexNotExists", any> = {
	meta: {
		type: "problem",
		docs: {
			description: "更好的模块化",
		...
		messages: {
			...
			indexNotExists: "'{{dir}}' not exists {{index}} file.",
		},
	},
	create(context) {
		// relative path
		...
		const options: ImportNoIndexOptions = ([...context.options] as ImportNoIndexOptions).sort((a, b) => a.dir > b.dir ? 1 : -1).map(option => {
			if (option.index) {
				...
				const indexPath = ["index.ts", "index.tsx", "index.js", "index.jsx"].find(i => Fs.existsSync(Path.join(option.dir, i)));
				if (!indexPath) {
					context.report({
						loc: {
							line: 0,
		...
		return {
			ImportDeclaration(importDeclaration) {
				...
				if (typeof importDeclaration.source.value === "string") {
					...
					// import 命中 dir
					const currentOption = options.find(option => importPath.indexOf(option.index!) === 0);
					if (currentOption) {
